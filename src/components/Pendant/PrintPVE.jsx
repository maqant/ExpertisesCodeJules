    ...
    <div className="flex flex-col h-full bg-slate-200 overflow-y-auto w-full p-8 items-center">
      ...
      <div className="bg-white w-full max-w-4xl p-12 shadow-xl print:shadow-none min-h-[1056px] text-slate-800" id="print-pve-container">
        ...
        <div className="grid grid-cols-2 gap-8 mb-8 border border-slate-300 p-4 rounded bg-slate-50">
          <div>
            <p><span className="font-bold">Assuré / Référence :</span> {formData.declarant || formData.nomResidence || '...'}</p>
            ...
            <p><span className="font-bold">Adresse :</span> {formData.adresse || '...'}</p>
          </div>
          <div>
            <p><span className="font-bold">Compagnie :</span> {formData.nomCie || '...'}</p>
            <p><span className="font-bold">Contrat :</span> {formData.nomContrat || '...'}</p>
            <p><span className="font-bold">N° de Police :</span> {formData.numPolice || '...'}</p>
            <p><span className="font-bold">N° Sinistre Cie :</span> {formData.numSinistreCie || '...'}</p>
          </div>
        </div>
          ...
          <table className="w-full text-sm border-collapse">
            <thead>
              <tr className="bg-slate-100 text-left">
                <th className="border p-2 w-1/3">Poste / Prestataire</th>
                <th className="border p-2 text-right">Réclamé</th>
                <th className="border p-2 text-center">Vétusté</th>
                <th className="border p-2 text-right">Retenu (PVE)</th>
              </tr>
            </thead>
            <tbody>
              {expenses.map(exp => (
                <tr key={exp.id} className="border-b">
                  <td className="border p-2">
                    ...
                    {exp.motifRefus && <div className="text-xs text-red-600 italic">Refusé : {exp.motifRefus}</div>}
                  </td>
                  <td className="border p-2 text-right">{exp.montantReclame} €</td>
                  <td className="border p-2 text-center">
                    {exp.pourcentageVetuste > 0 ? `${exp.pourcentageVetuste}%` : (exp.motifRefus ? 'Refus' : '-')}
                  </td>
                  <td className="border p-2 text-right font-bold">{exp.montantValide} €</td>
                </tr>
              ))}
            </tbody>
            <tfoot>
